...
func largestRectangleArea(heights []int) int {
	...
	for i := range heights {
		for len(stack) > 1 && heights[stack[len(stack)-1]] > heights[i] {
			...
			ans = max(ans, heights[idx]*(i-stack[len(stack)-1]-1))
		}
	...
	for len(stack) > 1 {
		idx := stack[len(stack)-1]
		stack = stack[:len(stack)-1]
		ans = max(ans, heights[idx]*(size-stack[len(stack)-1]-1))
	}
...
//	15.	for any bar, find it left & right boundary

//		if all bars in ascending order, then range will be previous index
//		to right most
...
//		the point is to find previous bar lower than self, and next bar
//		lower than self, which is stack property

//	16.	inspired from https://leetcode.com/problems/largest-rectangle-in-histogram/discuss/452612/Thinking-Process-for-Stack-Solution

//		author has a very good insight that, there are total n bars, so maximum
//		area only comes from those n bars, which yields to O(n) solution exist
